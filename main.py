...
def get_event_details_from_llm(text, ollama_config):
    """
    ...
    try:
        ...
        event_data = json.loads(response_json.get("response", "{}"))
        return event_data
...
def create_caldav_event(event_data, caldav_config):
    """
    ...
    try:
        ...
        if not calendar:
            ...
            print("Available calendars:", [c.name for c in principal.calendars()])
            return

        # Save the event
        print(f"Creating event in calendar '{calendar.name}'...")
        event_result = calendar.save_event(ical=cal.serialize())
...
def main():
    ...
    print("Please paste the text describing the event and press Ctrl+D (or Ctrl+Z on Windows) when you are done.")
    print("-" * 20)
    user_input = []
    while True:
        try:
    ...
    event_details = get_event_details_from_llm(text, config['ollama'])

    if event_details:
        print("\n--- Parsed Event Details ---")
        print(json.dumps(event_details, indent=2))
        print("--------------------------\n")
        create_caldav_event(event_details, config['caldav'])
